...
def log_conversion_failure(
    ...
    """Log conversion failures in a stable format for analyze_errors.py."""
    if exception is not None:
        logging.error(
            "Exception during conversion: %s (input: %s)", exception, input_path,
        )
        return
    stderr_snippet = stderr.strip()
    if len(stderr_snippet) > 2000:
        stderr_snippet = stderr_snippet[:2000] + "..."
    logging.error(
        "Error converting file, ffmpeg exited with code %s (input: %s): %s",
        exit_code,
        input_path,
...
def convert_file(
    ...
    if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
        ...
        if is_valid:
            ...
            print(
                f"WARNING: Existing output file {output_path} is corrupt or not h265. Re-encoding.",
            )
    ...
    if subtitle_streams:
        ...
        for _i, stream in enumerate(subtitle_streams):
    ...
    if audio_streams:
        for i, stream in enumerate(audio_streams):
            ...
            bitrate = (
                ...
                if stream.get("bit_rate", "").isdigit()
                else 0
            )
            bitrate_kb = bitrate // 1000 if bitrate > 0 else "unknown"

            print(
                f"Original Audio Stream {i}: {codec.upper()}, {channels}ch, {sample_rate}Hz, {bitrate_kb}kb/s",
            )
            ...
            if codec == "aac":
                ...
                cmd.extend(
                    [
                        ...
                        "192k",
                        f"-ac:{i}",
                        str(min(channels, 2)),  # Limit to stereo
                        f"-ar:{i}",
    ...
    if skip_subtitles:
        ...
        for sub_i, stream in enumerate(subtitle_streams):
            if is_mp4_container:
                ...
                print(
                    f"Subtitle stream {stream.get('index')}: Copying for MKV container",
                )
        cmd.extend(["-map", "0"])
    ...
    logging.info("Converting %s: %s", input_path, " ".join(cmd))
    ...
    try:
        ...
        if debug:
            ...
            if success:
                ...
                if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                    ...
                    size_reduction_pct = (
                        (size_diff_mb / input_size_mb) * 100 if input_size_mb > 0 else 0
                    )
                    ...
                    if not verify_media_file(output_path):
                        return False

                    conversion_succeeded = True
                    return True
                else:
                    msg = f"Error: Output file missing or empty: {output_path}"
                    print(msg)
                    logging.error(msg)
                    return False
            else:
                ...
                print(msg)
                log_conversion_failure(input_path, exit_code=result.returncode)
                return False
        else:
                ...
                if ready:
                    ...
                    if time.time() - last_activity_time > activity_timeout:
                        ...
                        if os.path.exists(output_path):
                            ...
                            print(
                                f"Current output size: {current_size / (1024 * 1024):.2f} MB",
                            )
                    time.sleep(0.1)
            ...
            # Get final results
            _, _ = current_process.communicate()
            if stderr_thread:
        ...
        log_conversion_failure(input_path, exception=e)
        return False
...
def main():
    ...
    parser.add_argument(
        "--hardware", action="store_true", help="Use hardware acceleration if available",
    )
    parser.add_argument(
